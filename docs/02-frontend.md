...
A diferencia de la sección anterior de "Recomendado" (que ahora alimenta "Inicio", ver [03-backend.md](03-backend.md)), `.discover_side` no es su propia caja separada: vive dentro de `.content_side`, un contenedor nuevo que envuelve tanto a `.song_side` como a `.discover_side` bajo un único borde/blur/esquinas redondeadas (antes eran dos cajas "vidriosas" independientes con un hueco entre medio). `.discover_side` ocupa toda la altura de esa caja compartida — por eso "Más de este artista" pide hasta 10 canciones (`MAX_ARTIST_TRACKS` en `server/routes/music.js`) en vez de 3, para no dejar espacio vacío — y usa `--color-panel-bg-strong` en vez de `--color-panel-bg` (el mismo "vidrio" que el resto de los paneles, pero más oscuro/opaco), separado de `.song_side` por un `border-left` sutil en vez de un hueco con doble borde.

### El reproductor (`master_play`) se ve pegado a `song_side`, pero es un elemento de grid aparte

Visualmente, el reproductor de abajo (`master_play`) se lee como el pie de la misma caja que `song_side` — mismo fondo, mismo borde, sin hueco entre los dos, y más angosto que antes (no llega hasta abajo de `discover_side`, termina justo donde termina `song_side`). Técnicamente, sin embargo, **no está anidado adentro de `.content_side`**: sigue siendo su propio elemento de grid (`grid-column: 2; grid-row: 2;`, con `margin-right: 300px` para no meterse debajo de `discover_side`).

La razón es `#admin-view`: ocupa el mismo lugar que `.content_side` (`grid-column: 2; grid-row: 1;`) cuando el usuario entra al panel de admin, y en CSS Grid dos elementos en la misma celda se apilan uno sobre el otro. Si `master_play` viviera adentro de `.content_side`, quedaría tapado por `#admin-view` cada vez que se muestra esa vista — rompiendo la reproducción persistente que se armó especialmente para que la música no se corte al entrar a Administración (ver [CHANGELOG.md](CHANGELOG.md), "Reproductor persistente al entrar a Administración"). Al ser un elemento separado en su propia fila del grid, `master_play` sigue arriba de todo pase lo que pase en la fila de arriba.

El efecto "misma caja" se logra con CSS, no con la estructura del HTML: `header` usa `row-gap: 0` (sin hueco entre `.content_side` y `.master_play`) y cada uno redondea solo las esquinas que no se tocan — `.content_side` tiene la esquina inferior izquierda cuadrada (`border-radius: var(--radius-lg) var(--radius-lg) var(--radius-lg) 0`) y sin borde inferior, `.master_play` tiene la esquina superior izquierda cuadrada (`border-radius: 0 0 0 var(--radius-lg)`) y sin borde superior — así donde se tocan no hay ni hueco ni línea doble, y las esquinas que sí quedan expuestas (arriba, y abajo a la izquierda) mantienen el redondeo del resto de los paneles. En mobile esto no aplica: cada panel vuelve a ser una caja completa y separada (con gap real entre ellos, como el resto de los paneles en ese layout), así que el media query repone el borde/redondeo completo y saca el `margin-right`.
