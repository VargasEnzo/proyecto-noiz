...
- **[discover.js](../js/dashboard/discover.js)**: la sección "Recomendado". Al arrancar el dashboard muestra un fallback (los 6 artistas con más canciones dentro del pool de populares que ya se cargó, sin pedir nada nuevo al servidor), y en paralelo pide `GET /api/music/recommendations` — si vuelve con canciones personalizadas (armadas a partir de las playlists y búsquedas del usuario, ver [03-backend.md](03-backend.md)), reemplaza el contenido y cada fila pasa a reproducir esa canción directo al clickearla, en vez de navegar a "Artistas" como hace el fallback. Se renderiza dos veces, una en la columna derecha (desktop) y otra dentro del cajón hamburguesa (mobile) — ver la sección de abajo.
