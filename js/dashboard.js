...
async function updateProfileRequest(datos) {
    const response = await fetch('/api/me', {
        method: 'PUT',
...
function renderSidebarProfile() {
    greeting.textContent = currentUser.nombre;
    if (currentUser.avatar) {
        sidebarAvatarImg.src = currentUser.avatar;
...
function openProfileModal() {
    ...
    profileApellidoInput.value = currentUser.apellido;
    profileEmailDisplay.value = currentUser.email;
    profilePlanBadge.textContent = currentUser.plan;
    profileCreatedAt.textContent = `Miembro desde el ${new Date(currentUser.created_at).toLocaleDateString('es-AR', {
        day: 'numeric',
        month: 'long',
        year: 'numeric',
    })}`;
    showProfileAvatar(currentUser.avatar);
...
document.getElementById('profile-save-btn').addEventListener('click', async () => {
    ...
    currentUser = await updateProfileRequest(datos);
    renderSidebarProfile();
    closeProfileModal();
});
